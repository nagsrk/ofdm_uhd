...
def run_transmiter(n,nbytes,count,carrier_map,tb,norestart,pkt_size):    
	...
	if string.count(carrier_map,'0') >= 1000:
	...
	if (norestart.value==1):
	      ...
	      while n < nbytes:
			...
			if data == '':
				...
				payload = struct.pack('!H', pktno & 0xffff) + data # Constructing a packet. Pack a string in to given format. http://docs.python.org/library/struct.html
				send_pkt(tb,payload)		
				sys.stderr.write('.')
				pktno += 1
			    break;
