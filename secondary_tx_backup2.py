...
import string
import time, struct,sys, math, os
import ofdm
...
def synchronization(tb1):    
	tb1.start()                       # start flow graph
	n= 0
	pktno = 1000
	print "\nTransmitting in", Freq.value ,"Hz", " containing synchronization packet"
	preamble = 11111
	while n < 50:
	      data = struct.pack('!H', Frequency.value & 0xffff)
	      ...
	      send_pkt(tb1,payload)				     # Sending packet
	      n += 1
...
class trans_init():
	...
	parser.add_option("-s", "--size", type="eng_float", default=2048, help="set packet size [default=%default]")
	...
	parser.add_option("", "--dwell-delay", type="eng_float",default=2e-3, metavar="SECS", help="time to dwell (in seconds) at a given frequency[default=%default]")
...
def sense_loop(tb,count,Frequency):
	...
	if avg_iter_count > 0:
		...
		thrshold = map(lambda x: 0 if x>0.2 else 1, moving_avg_data)		# Modified threshold	
...
def run_transmiter(n,nbytes,count,carrier_map,tb,norestart,pkt_size,sync):    
	tb.start()                       # start flow graph
	print "\n\nCarrier map = ",carrier_map
	if string.count(carrier_map,'0') >= 0:
	...
	if (norestart.value==1):
	      ...
	      source_file = open("/home/kranthi/tx.txt",'r') # Source file location	# Added Line
...
def Start_transmitter(Freq,norestart,sync,Frequency):             
	  ...
	  if Freq.value == 920*10**6:
		...
		n=0
		while(n < nbytes):
			carrier_map_new = sense_loop(tb1,count1,Frequency)
			n = run_transmiter(n,nbytes,count,carrier_map_new,tb,norestart,pkt_size,sync)
		send_pkt(tb,"", carrier_map_new, eof = True)                     
                
if __name__ == '__main__':
	try:
	  Frequency = Value('i',900*10**6)
	  sync = Value('i',1)
	  norestart = Value('i',1)
