...
def transmitter_control(mp,Freq,norestart):
	...
	mp.terminate()
	f_set=Freq.value
	#while(f_set == Freq.value):
	      #Freq.value =  random.randrange(800*10**6,1000*10**6,10*10**6)
	if(f_set==940*10**6):
	    Freq.value=900*10**6
	elif(f_set==920*10**6):
	    Freq.value = 940*10**6
...
def run_transmiter(n,nbytes,count,carrier_map,tb,norestart,pkt_size):    
	...
	source_file = open("/home/kranthi/project/file1")
        while n < nbytes:
	      if (count==0):
		      break;
...
def Start_transmitter(Freq,norestart):             
	  ...
	  count=20 #number of packets to send before sensing
...
if __name__ == '__main__':
	try:
	  ...
	  #Freq=Value('i',random.randrange(870*10**6,930*10**6,5*10**6))
	  mp=Process(target=Start_transmitter,args=(Freq,norestart))
	  mp.start()
