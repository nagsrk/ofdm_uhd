...
import usrp_transmit_path1
from multiprocessing import Process,Value

class transmit(gr.top_block):		#top block for TX
...
class transmit1(gr.top_block):		#top block for sensor
	def __init__(self, options):
	    gr.top_block.__init__(self)
	    self.txpath = usrp_transmit_path1.usrp_transmit_path(options)
	    self.connect(self.txpath)
	...
	def _print_verbage(self):
	    ...
	    fd=open('string_pipe','r')
	    string=fd.read()
	    usrp_transmit_path1.string_pass_l2(string)
...
class trans_init():				# For transmitter
...
class trans_init1():					# For receiver
    def __init__(self,Freq): 
        global parser
        parser = OptionParser(option_class=eng_option, conflict_handler="resolve")
	expert_grp = parser.add_option_group("Expert")
	...
	usrp_transmit_path1.add_options(parser, expert_grp)
	ofdm1.ofdm_mod.add_options(parser, expert_grp)
	(self.options, self.args) = parser.parse_args ()
	if len(self.args) != 0:
		parser.print_help()
		sys.exit(1)
	if self.options.tx_freq is None:
		...
		sys.exit(1)
	self.options.tx_freq=Freq.value
	self.tb = transmit1(self.options)
	r = gr.enable_realtime_scheduling()
	if r != gr.RT_OK:
		print "Warning: failed to enable realtime scheduling"
